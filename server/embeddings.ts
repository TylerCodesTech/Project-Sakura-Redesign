...
export async function findSimilarTicketsAndDocs(description: string): Promise<{
  ...
  try {
    ...
    const similarTickets = await db.select({
      ...
      distance: cosineDistance(tickets.embedding, queryEmbedding),
    })
    .from(tickets)
    .where(isNotNull(tickets.embedding))
    .orderBy(cosineDistance(tickets.embedding, queryEmbedding))
    .limit(10);

    const similarDocs = await findSimilarDocuments(queryEmbedding, 5, 0.3);
    ...
    return {
      tickets: similarTickets.map(t => ({
        ...
        similarity: 1 - (t.distance as number),
      })).filter(t => t.similarity >= 0.3),
      docs: similarDocs.map(d => ({
        id: d.id,
        title: d.title,
    ...
    console.error("Error finding similar tickets and docs:", error);
    return { tickets: [], docs: [] };
  }
}

export async function searchTicketsBySemantic(query: string): Promise<Array<{
  id: string;
  title: string;
  ...
  try {
    ...
    return results
      .filter(r => (1 - (r.distance as number)) >= 0.25)
      .map(r => ({
        id: r.id,
        title: r.title,
    ...
    console.error("Error in semantic ticket search:", error);
    return [];
  }
}
