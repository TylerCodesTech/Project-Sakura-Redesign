...
- **Redesigned Helpdesk Page**: Four-section layout with Dashboard (SLA stats, quick actions, recent activity), My Queue (personally assigned tickets), Team Queue (unassigned + team tickets), and Teams (hierarchical department/sub-team navigation). Supports ticket routing workflow where new tickets land in root department pool, get auto-routed to sub-teams based on category, and are either auto-assigned or picked up by agents.
- **Department Hierarchy**: Departments now support parent-child relationships via `parentId` field, enabling sub-team structures (e.g., IT Support → Helpdesk, Hardware, Sys Admin, Network & Security).
